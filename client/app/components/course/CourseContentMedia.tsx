...
const CourseContentMedia: FC<Props> = ({
    ...
    const handleSubmit = async (action: any, payload: any, successMessage: string) => {
        try {
    ...
    const handleQuestion = async () => {
        if (!question) {
            showToastError("Question can't be empty!");
        } else {
            await handleSubmit(addNewQuestion, {
                question,
                courseId: id,
    ...
    const handleSubmitAnswer = async () => {
        await handleSubmit(addAnswerInQuestion, {
            answer,
            courseId: id,
            contentId: data[activeVideo]._id,
    ...
    const handleSubmitReplyReview = async () => {
        if (replyReviewLoading) return;

        if (!reply) {
            showToastError("Reply can't be empty!");
        } else {
            await handleSubmit(addReplyReviewInCourse, {
                comment: reply,
                courseId: id,
                reviewId: reviewId
    ...
    useEffect(() => {
        ...
        // Handle errors
        [error, isError, hasError, canError].forEach(handleError);
    }, [
        isSuccess,
        hasSuccess,
        wasSuccess,
